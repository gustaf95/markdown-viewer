...
| 5 | DOCX 가져오기 | 중간 | 없음 | **구현 완료** (수식 = OMML -> LaTeX) |
| 6 | HWPX 가져오기 | 높음 | 없음 | **구현 완료** (수식 = 한글 수식 스크립트 -> LaTeX) |
| 7 | HTML 가져오기 | 낮음 | 없음 | 계획 |
| 8 | .hwp / .doc 가져오기 | 중간 | 한글·Word (변환용) | 계획 — 3.1 참고 |

> 내보내기 세 형식과 가져오기 두 형식이 구현되었다.
> 내보내기는 DocModel에서 여러 형식으로 갈라지고, 가져오기는 여러 형식이 DocModel로 모인다.
> 수식은 어느 방향이든 이미지로 떨어뜨리지 않는다 — 이 프로젝트의 기준선이다.
...
| 수식 크기 | `<hp:sz>`와 `baseLine`은 **한글이 열 때 다시 계산하지 않는다**. 수식을 편집해야 갱신된다 |
| 위 항목 정정 | 처음엔 "한글이 다시 계산한다"고 봤는데, PDF 저장이 전체 재조판을 강제해서 그렇게 보였을 뿐이다. 화면 표시는 저장된 값을 그대로 쓴다 |
...
**수식 상자 크기 — 어림이 아니라 계산해야 한다**

`<hp:equation>`은 상자의 폭·높이와 기준선 위치(`baseLine`, 높이에 대한 %)를 갖는다.
한글은 이 값을 파일을 열 때 **다시 계산하지 않으므로**, 어긋나면 인라인 수식이 글줄 위로 떠 보인다.
(수식을 편집하면 한글이 갱신해서 정상으로 돌아온다 — 이 증상으로 원인을 찾았다)

처음엔 높이를 글자 크기의 2.4배로 고정했는데, 평범한 인라인 수식에는 2.6배나 큰 값이었다.
지금은 MathML 구조를 훑어 글자 크기(em) 단위로 위/아래 높이를 계산한다 (`measureMath`).

상수는 **한글이 실제로 조판한 값과 맞춰 보정**했다. `EquationCreate`로 같은 스크립트를
한글에 넣고 저장하면 한글이 계산한 참값이 파일에 남으므로, 그것을 정답지로 썼다.

한글은 같은 수식에도 **경로에 따라 다른 상자**를 준다. `EquationCreate`로 넣으면 딱 맞는 값을,
수식 편집기로 열었다 저장하면 7% 정도 넉넉한 값을 쓴다. 사용자가 편집기로 손보면 후자로 바뀌므로
그쪽에 맞춰 뒀다 — 그래야 나중에 수식을 건드려도 글줄 높이가 흔들리지 않는다.

| | 처음 | 지금 |
|---|---|---|
| 높이 (편집기 보정본 대비) | 0.34 ~ 2.7배 | **0.95 ~ 1.03배** (평균 1.00) |
| 폭 | 0.92 ~ 1.14배 | **0.96 ~ 1.03배** (평균 1.01) |
| `baseLine` | 63 고정 | 62~88 (보정본과 최대 4 차이) |

**높이**에서 알아낸 것

- 평범한 한 줄 0.93em/기준선 86%, 윗첨자 있으면 1.11em/89%, 분수·적분 포함이면 2.1~2.9em/62~71%
- `∫`는 글리프 자체가 크고 한계값이 옆에 붙는다. `∑`·`∏`는 글리프가 작은 대신
  한계값이 위아래로 쌓인다 — 둘을 다르게 재야 한다
- `∑`는 MathML이 옆첨자(`msub`/`msup`)로 주더라도 한글 스크립트에서는 위아래로 쌓인다.
  MathML이 아니라 **우리가 내보내는 스크립트를 기준으로** 재야 한다

**폭**에서 알아낸 것 — 글자 폭은 실측표를 쓴다

한 줄에 같은 글자를 20개씩 넣어 한글에 조판시키고 폭을 나누면 글자별 실제 폭이 나온다.
HancomEQN은 **1/28em 격자**를 쓴다:

```
l=0.339  i=0.357  c=0.429  a=0.500  n=0.571  m=0.786  M=0.929
=0.500   +=0.786  (=0.357  <=0.350   ≤ ≥ ≠ ≈ ∞ ∇ → = 1.000
윗첨자 배율 0.714 · 근호 여백 0.805 · 분수 여백 0.476
```

처음엔 모든 글자를 0.58em으로 뭉뚱그렸는데 `l`(0.34)과 `M`(0.93)은 2.7배 차이가 난다.
`=`도 넓은 줄 알고 0.92로 뒀지만 실제로는 0.5다 (여백이 넓을 뿐 글리프는 좁다).

여백 규칙에서 틀렸던 것:

- 연산자 판별이 ASCII만 봐서 `−`(U+2212)·`±`·`×`를 좁은 글자로 세고 있었다.
  연산자가 많은 수식일수록 폭이 모자랐다 (근의 공식은 18% 부족)
- `~`(빈칸)는 0.25em이 아니라 **0.5em**이다. `~` 하나만 다른 수식 두 개의 폭 차이로 확인했다
- 줄 맨 앞의 `+`/`-`는 **단항**이라 앞뒤 여백이 붙지 않는다 (`-b`는 뺄셈이 아니다)

- [ ] **글자 크기별 재검증**: 실측은 전부 10.5pt(baseUnit 1050)에서 했다.
      제목처럼 크기가 다른 곳에서도 같은 비율인지 확인이 필요하다. 한글이 글자마다
      정수 HWPUNIT로 반올림한다면 작은 크기에서 오차가 커질 수 있다.
      아직 아주 살짝 어긋나는 것이 남아 있어 이 방향을 확인해 볼 만하다.
...
설계는 [doc/README.md 18장](README.md#18-가져오기import-기능)에 정리했다.

- [x] **DOCX 가져오기** — OMML을 LaTeX으로 되돌린다
- [x] **HWPX 가져오기** — 한글 수식 스크립트를 LaTeX으로 되돌린다
- [ ] HTML 가져오기 — KaTeX가 `annotation`에 원본 LaTeX을 남겨 두므로 가장 쉽다
- [ ] 남이 만든 문서 대응 넓히기 — 지금은 우리가 내보낸 문서만 확실히 되읽는다.
      제목·목록·코드 블록은 한글/Word 문서에 표시가 없어 모양으로 추정하는데,
      문서마다 규칙이 달라 안 맞는 경우가 있다
- [ ] 이미지를 `<문서명>_files/` 폴더로 빼내기 (F-1206) — 지금은 data URI로 본문에 박혀 문서가 커진다

### 3.1 이진 형식(.doc / .hwp) — 변환해서 기존 경로에 태우기

`.doc`와 `.hwp`는 둘 다 OLE2 복합 파일이라 직접 파싱하는 비용이 매우 크다.
대신 **오피스 프로그램으로 열린 형식으로 바꾼 뒤 이미 만든 가져오기에 태우는** 방식이 낫다.

```
.doc  --(Word 또는 한글 COM)-->  .docx  --> 기존 DOCX 가져오기
.hwp  --(한글 COM)------------>  .hwpx  --> 기존 HWPX 가져오기
```

- [ ] `.hwp` 지원 — **이쪽을 먼저 한다.** 한글이 자기 형식끼리 변환하는 것이라
      수식이 한글 수식 스크립트 그대로 남을 가능성이 높고, 그러면 이미 만든 파서가 그대로 먹힌다
- [ ] `.doc` 지원 — 위가 되면 같은 뼈대에 얹는다

**미리 알아 둘 함정: 컨테이너를 바꿔도 수식은 안 바뀔 수 있다.**

`.doc`의 수식은 대개 Equation 3.0 / MathType **OLE 개체(MTEF 이진)**다.
Word가 `.docx`로 저장할 때 이것을 OMML로 바꿔 주지 않고 OLE 개체 그대로 옮긴다
("Office Math로 변환"은 사용자가 따로 눌러야 하는 별개 명령이다).
그러면 우리 importer에는 `<m:oMath>`가 없으니 수식이 그림 덩어리로 남거나 비어 버린다.
글자·표·서식은 잘 넘어오고 **수식만 안 넘어올 것으로 예상**한다 — 확인이 필요하다.

설계 시 지킬 것:

- 원본은 절대 건드리지 않는다. 임시 폴더에서만 변환하고 끝나면 지운다
- 변환기(Word·한글)가 없으면 "Word/한글에서 .docx(.hwpx)로 저장한 뒤 가져오세요"로 안내한다.
  외부 바이너리를 동봉하지 않는다는 결정은 그대로다 (LibreOffice headless는 700MB라 기각)
- 변환 경로로 들어온 문서는 "수식이 그림으로 들어왔을 수 있습니다"를 함께 알린다
- Pandoc은 `.docx`만 읽는다. `.doc`에는 쓸 수 없다
