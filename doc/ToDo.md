...
| | 처음 | 지금 |
|---|---|---|
| 높이 (편집기 보정본 대비) | 0.34 ~ 2.7배 | **0.93 ~ 1.03배** (평균 1.00) |
| 폭 | — | **0.92 ~ 1.10배** (평균 1.00) |
| `baseLine` | 63 고정 | 62~88 (보정본과 최대 4 차이) |

폭에서 오래 틀렸던 것: 연산자 판별이 ASCII만 봐서 `−`(U+2212)·`±`·`×`를 좁은 글자로 세고 있었다.
연산자가 많은 수식일수록 폭이 모자랐다 (근의 공식은 18% 부족).

- 평범한 한 줄: 높이 0.93em, 기준선 86%
- 윗첨자 있음: 1.11em, 89%
- 분수·적분 포함: 2.1~2.9em, 62~71%
