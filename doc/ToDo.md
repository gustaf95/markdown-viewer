...
| 확인한 것 | 내용 |
|---|---|
| 유니코드 기호 | `α ∫ ≤ × → ∂` 등을 **그대로 써도 인식**된다. 기호 이름표를 따로 둘 필요가 없다 |
| 큰 연산자 | 단, `∑`·`∏`는 유니코드로 두면 한계값이 **옆에** 붙는다. `sum`·`prod` 이름표로 바꿔야 위아래로 간다 |
...
| `msup`/`msub`/`msubsup` | `^{}` / `_{}` / `_{} ^{}` | |
| `munderover` + ∑·∫ | `sum _{} ^{}` | 큰 연산자는 이름표로 치환 |
| `munder` + lim | `lim _{}` | 함수 이름은 중괄호로 감싸지 않는다 |
